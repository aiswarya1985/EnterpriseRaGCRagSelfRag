...
def grade_chunks(question: str, chunks: list[RetrievedChunk]) -> CRAGEvaluation:
    logger.info("Entering grade_chunks function")
    ...
    try:
        ...
        logger.info(f"Grading result: {parsed}")
...
def crag_pipeline(
    ...
    logger.info("Entering crag_pipeline function")
    if not enable_crag:
        return (
            chunks,
            CRAGEvaluation(
    ...
    if not chunks:
        ...
        try:
            logger.info("chunks retrieved, triggering web search") 
